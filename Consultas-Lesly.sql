SELECT DISTINCT T5.COD_CIA, T5.TIP_DOCUM, T5.COD_DOCUM, T5.NOMBRE_CONTR, T5.COD_RAMO, T5.NUM_POLIZA,T5.NOM_RAMO, E.NOM_RIESGO, T5.NUM_SPTO, T5.NUM_APLI, T5.FEC_EFEC_SPTO,
T5.FEC_VCTO_SPTO, T5.FEC_ACTU, T5.TIP_SPTO, T5.TXT_MOTIVO_SPTO, T5.MCA_PROVISIONAL, T5.MCA_POLIZA_ANULADA, T5.SITUACION, T5.COD_MON, T5.PRIMA_TOTAL, T5.PRIMA_PAGADA
FROM
...
T4.FEC_VCTO_SPTO, T4.FEC_ACTU, T4.TIP_SPTO, T4.TXT_MOTIVO_SPTO, T4.MCA_PROVISIONAL,T4.MCA_POLIZA_ANULADA, T4.SITUACION, T4.PRIMA_TOTAL, T4.COD_MON, SUM(D.IMP_RECIBO) PRIMA_PAGADA
FROM
...
T3.FEC_VCTO_SPTO, T3.FEC_ACTU, T3.TIP_SPTO, T3.TXT_MOTIVO_SPTO, T3.MCA_PROVISIONAL, T3.MCA_POLIZA_ANULADA, T3.SITUACION, C.COD_MON, SUM(C.IMP_RECIBO) PRIMA_TOTAL
FROM
...
T2.NUM_SPTO, T2.NUM_APLI, T2.FEC_EFEC_SPTO, T2.FEC_VCTO_SPTO, T2.FEC_ACTU, T2.TIP_SPTO, T2.TXT_MOTIVO_SPTO, T2.MCA_PROVISIONAL,T2. MCA_POLIZA_ANULADA, T2.SITUACION
FROM
...
T1.FEC_ACTU, T1.TIP_SPTO, T1.TXT_MOTIVO_SPTO, T1.MCA_PROVISIONAL,T1.MCA_POLIZA_ANULADA, T1.SITUACION
FROM
(SELECT COD_CIA, COD_RAMO, NUM_POLIZA, TIP_DOCUM, COD_DOCUM, NUM_SPTO, NUM_APLI, FEC_EFEC_SPTO, FEC_VCTO_SPTO, FEC_ACTU, TXT_MOTIVO_SPTO, TIP_SPTO, MCA_PROVISIONAL,MCA_POLIZA_ANULADA,
...
ELSE 'EXPIRADA'
END SITUACION
FROM A2000030 WHERE COD_DOCUM IN(
'47059932'
)) T1
...
T3.FEC_VCTO_SPTO, T3.FEC_ACTU, T3.TIP_SPTO, T3.TXT_MOTIVO_SPTO, T3.MCA_PROVISIONAL, T3.MCA_POLIZA_ANULADA, T3.SITUACION, C.COD_MON) T4
...
T4.FEC_VCTO_SPTO, T4.FEC_ACTU, T4.TIP_SPTO, T4.TXT_MOTIVO_SPTO, T4.MCA_PROVISIONAL,T4.MCA_POLIZA_ANULADA, T4.SITUACION, T4.PRIMA_TOTAL, T4.COD_MON) T5
